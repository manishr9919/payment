...
const api = axios.create({
  baseURL:
    ...
    "https://assignment-evidron-school-payment-backend.onrender.com/api",
});
...
export const getTransactionsBySchool = async (schoolId, params) => {
  try {
    ...
    const response = await api.get(
      `/payments/transactions/school/${schoolId}`,
      { params }
    );
    return response.data;
  } catch (error) {
    console.error(
      "Error fetching transactions by school:",
      error.message,
      error.response?.data
    );
    throw error;
...
export const getTransactionStatus = async (customOrderId) => {
  try {
    const response = await api.get(
      `/payments/transaction-status/${customOrderId}`
    );
    return response.data;
  } catch (error) {
