  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://assignment-evidron-school-payment-backend.onrender.com/api/auth/login",
        formData
      );
      ...
      if (response.data.token) {
        ...
        // Optional: configure axios to always send this token
        axios.defaults.headers.common[
          "Authorization"
        ] = `Bearer ${response.data.token}`;

        // Update token state in parent (if needed)
        setToken(response.data.token);
